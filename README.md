...
| **macOS Ventura**:<ul><li>Requires a custom BIOS (like 1vyrain) with CFG Lock disabled in order to re-enable ACPI CPU Power Management. Using the `AppleCpuPmCfgLock` Quirk is not enough!<li> You can upgrade to macOS 13.3 beta now and use the latest version OpenCore Legacy Patcher (0.6.2) to install the Intel HD4000 drivers.</li>
